...
function defaultsForLocale(locale: SupportedLocale): Omit<
  ResolvedRangeLocaleText,
  'presetOptions' | 'flexDatesOptions'
> {
...
function mergeFlexDatesOptions(
  ...
  return FLEX_VALUES.map((value) => ({
    value,
    label: overrides[value] ?? base.find((option) => option.value === value)!.label,
  }))
}
...
export function resolveRangeLocaleText(
  ...
  return {
    ...
    flexDatesPlaceholder: overrides?.flexDatesPlaceholder ?? base.flexDatesPlaceholder,
    ...
    invalidFormatStart: overrides?.invalidFormatStart ?? base.invalidFormatStart,
    ...
    flexDatesOptions: mergeFlexDatesOptions(resolvedLocale, overrides?.flexDates),
  }
}
