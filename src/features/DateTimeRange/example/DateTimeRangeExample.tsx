...
import { DateTimeRange, getMaxEndForStart } from '../index'
...
function formatRangeValue(value: DateTimeRangeValue, mode: 'date' | 'datetime') {
...
export function DateTimeRangeExample() {
  ...
  const [datetimeRange, setDatetimeRange] = useState<DateTimeRangeValue>({    start: new Date(),
    end: new Date(),
  })
    ...
    <>
      ...
      <section className="dtr-example">
        ...
        <div className="dtr-example-stack">
          ...
          <div className="dtr-example-block">
            <DateTimeRange
              ...
              endLabel="Data do"
            />
            <p className="selected-value">Wartość: {formatRangeValue(dateRange, 'date')}</p>
          </div>
          ...
          <div className="dtr-example-block">
            ...
            />
            <p className="selected-value">
              Wartość: <code>{formatRangeValue(datetimeRange, 'datetime')}</code>
            </p>
          </div>

          <div className="dtr-example-block">
            <DateTimeRange            value={preciseRange}
          ...
          <p className="selected-value">
            Start UTC:{' '}
            <code>{preciseRange.start ? serializeBackendUtc(preciseRange.start) : 'brak'}</code>
          </p>
          <p className="selected-value">
            Koniec UTC:{' '}
            <code>{preciseRange.end ? serializeBackendUtc(preciseRange.end) : 'brak'}</code>
          </p>
        </div>
        ...
        <div className="dtr-example-block">
          ...
          />
          <p className="selected-value">
            Wartość: {formatRangeValue(limitedRange, 'date')} — limit 3 dni kalendarzowe
          </p>
        </div>
        ...
        <div className="dtr-example-block">
          ...
          />
          <p className="selected-value">
            Wartość: <code>{formatRangeValue(precisionRange, 'datetime')}</code>
          </p>
          <p className="selected-value">
            Start UTC:{' '}
            <code>
              {precisionRange.start ? serializeBackendUtc(precisionRange.start) : 'brak'}
            </code>
          </p>
          <p className="selected-value">
            Max koniec UTC (2 doby od startu): <code>{formatMaxEnd(precisionRange.start)}</code>
          </p>
          <p className="selected-value">
        ...
        <div className="dtr-example-block">
          ...
          />
          <p className="selected-value">
            Wartość: <code>{formatRangeValue(hoursRange, 'datetime')}</code> — limit 6 godzin
          </p>
        </div>
      </div>
    </section>
