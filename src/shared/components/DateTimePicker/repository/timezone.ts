...
export function getMinutes(date: Date, timezone: DateTimePickerTimezone): number {
    return timezone === "UTC" ? date.getUTCMinutes() : date.getMinutes();
}

export function getSeconds(date: Date, timezone: DateTimePickerTimezone): number {
    return timezone === "UTC" ? date.getUTCSeconds() : date.getSeconds();
}

export function getMilliseconds(date: Date, timezone: DateTimePickerTimezone): number {
...
export function createInstant(
    ...
    if (timezone === "UTC") {
        return new Date(Date.UTC(fields.year, fields.month, fields.date, hours, minutes, seconds, milliseconds));
    }

    return new Date(fields.year, fields.month, fields.date, hours, minutes, seconds, milliseconds);
}
...
export function setDatePartTz(base: Date | null, day: Date, timezone: DateTimePickerTimezone): Date {
    if (timezone === "UTC") {
    ...
    result.setFullYear(getYear(day, timezone), getMonth(day, timezone), getDate(day, timezone));
    if (!base) {
...
export function setTimePartTz(base: Date | null, hours: number, minutes: number, seconds = 0, timezone: DateTimePickerTimezone, milliseconds = 0): Date {
    if (!base) {
...
export function withoutSecondsTz(date: Date, timezone: DateTimePickerTimezone): Date {
    if (timezone === "UTC") {
        return createInstant(
            {
...
export function withoutMillisecondsTz(date: Date, timezone: DateTimePickerTimezone): Date {
    if (timezone === "UTC") {
        return createInstant(
            {
                year: getYear(date, timezone),
...
export function isSameDayTz(a: Date, b: Date, timezone: DateTimePickerTimezone): boolean {
    return getYear(a, timezone) === getYear(b, timezone) && getMonth(a, timezone) === getMonth(b, timezone) && getDate(a, timezone) === getDate(b, timezone);
}

export function isSameMonthTz(a: Date, b: Date, timezone: DateTimePickerTimezone): boolean {
    return getYear(a, timezone) === getYear(b, timezone) && getMonth(a, timezone) === getMonth(b, timezone);
}

export function isSameYearTz(a: Date, b: Date, timezone: DateTimePickerTimezone): boolean {
    return getYear(a, timezone) === getYear(b, timezone);
}

export function startOfDayTz(date: Date, timezone: DateTimePickerTimezone): Date {
    return createInstant(
        {
            year: getYear(date, timezone),
            month: getMonth(date, timezone),
...
export function addYearsTz(date: Date, amount: number, timezone: DateTimePickerTimezone): Date {
    if (timezone === "UTC") {
        return createInstant(
            {
...
export function setMonthPartTz(base: Date, monthIndex: number, timezone: DateTimePickerTimezone): Date {
    if (timezone === "UTC") {
        return createInstant(
            {
                year: getYear(base, timezone),
...
export function setYearPartTz(base: Date, year: number, timezone: DateTimePickerTimezone): Date {
    if (timezone === "UTC") {
        return createInstant(
            {
                year,
...
export function buildCalendarDaysTz(month: Date, showOutside: boolean, timezone: DateTimePickerTimezone): Date[] {
    ...
    const startOffset = timezone === "UTC" ? (first.getUTCDay() + 6) % 7 : (first.getDay() + 6) % 7;

    const days: Date[] = [];
    for (let i = 0; i < 42; i += 1) {
        if (timezone === "UTC") {
            days.push(createInstant({ year, month: monthIndex, date: 1 - startOffset + i }, timezone));
            continue;
        }
    ...
    if (!showOutside) {
        ...
        for (let i = days.length - 1; i >= 0; i--) {
            if (isSameMonthTz(days[i], month, timezone)) {
                lastNeeded = i;
                break;
            }
        }
...
//ISO z backendu (z lub bez Z) instant
export function parseBackendUtc(iso: string): Date {
    ...
    return new Date(Date.UTC(Number(ys), Number(ms) - 1, Number(ds), Number(hs), Number(mins), Number(ss), milliseconds));
}
