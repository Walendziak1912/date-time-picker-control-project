...
import { useDateTimePickerController, usePopoverDismiss, usePopoverPlacement } from "../hooks";
import type { DateTimePickerHandle, DateTimePickerProps, DateTimePickerView } from "../types";
import "./DateTimePicker.css";

export const DateTimePicker = forwardRef<DateTimePickerHandle, DateTimePickerProps>(function DateTimePicker(props, ref) {
    const {
        label,
        name,
        className,
  ...
  useImperativeHandle(ref, () => ({ validate, reset }), [validate, reset]);
        ...
        >
            ...
            <div className="dtp-field">
                ...
                {!disabled && !readOnly && (
                    <button
                        type="button"
                        className={["dtp-clear", value || inputText ? "" : "dtp-clear--hidden"].filter(Boolean).join(" ")}
            ...
            {open && (
                ...
                >
                    ...
                    <div className={["dtp-views", showCalendar && !showTime ? "dtp-views--date-only" : "", !showCalendar && showTime ? "dtp-views--time-only" : ""].filter(Boolean).join(" ")}>
                        {showCalendar && (
                            <Calendar
                                month={month}
                                value={draft}
