...
import { usePopoverDismiss, usePopoverPlacement } from "../hooks";
import type { useDateTimePickerController } from "../hooks/useDateTimePickerController";
import type {
  DateTimePickerSharedProps,
  DateTimePickerView,
} from "../types";
import "./DateTimePicker.css";

type DateTimePickerShellProps = {
  shellProps: Pick<
    DateTimePickerSharedProps,
    | "label"
    | "name"
    ...
    | "onYearChange"
  >;
  controller: ReturnType<typeof useDateTimePickerController>;
  testId: string;
};

export function DateTimePickerShell({
  shellProps,
  controller,
  testId,
}: DateTimePickerShellProps) {
  const {
    label,
    name,
    className,
    ...
    >
      ...
      <div className="dtp-field">
        ...
        {!disabled && !readOnly && (
          <button
            type="button"
            className={["dtp-clear", value || inputText ? "" : "dtp-clear--hidden"]
              .filter(Boolean)
              .join(" ")}
