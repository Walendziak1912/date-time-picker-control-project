...
const PRESET_LABELS: Record<
  SupportedLocale,
  Record<DateTimeRangePresetKey, string>
> = {
  "pl-PL": {
    today: "Dzisiaj",
    yesterday: "Wczoraj",
...
export function buildDefaultPresetOptions(
  locale?: string,
): DateTimeRangePresetOption[] {
...
function endOfWeekTz(date: Date, timezone: DateTimePickerTimezone): Date {
  return endOfDayTz(
    addDaysTz(startOfWeekTz(date, timezone), 6, timezone),
    timezone,
  );
}
...
export function getPresetRange(
  ...
  switch (preset) {
    ...
    case "thisWeek":
      return {
        start: startOfWeekTz(now, timezone),
        end: endOfWeekTz(now, timezone),
      };
    case "lastWeek": {
    ...
    case "thisMonth":
      return {
        start: startOfMonthTz(now, timezone),
        end: endOfMonthTz(now, timezone),
      };
    case "lastMonth": {
      ...
      return {
        start: startOfMonthTz(prevMonth, timezone),
        end: endOfMonthTz(prevMonth, timezone),
      };
    }
    default:
