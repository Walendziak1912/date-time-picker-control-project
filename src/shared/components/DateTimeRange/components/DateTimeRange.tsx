...
export const DateTimeRange = forwardRef<DateTimeRangeHandle, DateTimeRangeProps>(function DateTimeRange(props, ref) {
    const {
    ...
    useImperativeHandle(
        ...
        () => ({
            ...
            reset: () => {
                startPickerRef.current?.reset();
                endPickerRef.current?.reset();
                resetValidation();
            },
        }),
        [validateFields, resetValidation],
    );
    ...
    const layoutModifiers = [showPresets ? "dtr--with-presets" : "", showFlexDates ? "dtr--with-flex-dates" : ""].filter(Boolean).join(" ");
